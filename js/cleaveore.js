...
{    
	...
	for(var i in e.detail.Encounter)
	{
		...
		e.detail.Encounter[i] = ('' + e.detail.Encounter[i]);
...
advjob = [ "PLD", "PLD", "WAR", "MNK", "DRG", "NIN", "BRD", "BLM", "SMN", "WHM", "SCH", "MCH", "DRK", "AST", "SAM", "RDM", "GNB", "DNC", "SGE", "RPR" ],
tanker = [ "PLD", "WAR", "DRK", "GNB" ],
...
function domReady() 
{
	...
	try
	{
		webs = new ActWebsocketInterface(wsUri);
		webs.connect();
	}
	catch(ex)
	{
